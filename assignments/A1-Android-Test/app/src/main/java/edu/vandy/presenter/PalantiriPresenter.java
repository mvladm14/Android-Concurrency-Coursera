...
public class PalantiriPresenter
     ...
     */
    private List<edu.vandy.presenter.BeingThread> mBeingsThreads;
     ...
     */
    private List<edu.vandy.view.DotArrayAdapter.DotColor> mPalantiriColors =
            new ArrayList<edu.vandy.view.DotArrayAdapter.DotColor>();

    /**
     * This List keeps track of how many beings we have and whether
     * they're gazing or not.
     */
    private List<edu.vandy.view.DotArrayAdapter.DotColor> mBeingsColors =
            new ArrayList<>();
    ...
    public void onCreate(MVP.RequiredViewOps view) {
        ...
        // in the intent.
        if (edu.vandy.utils.Options.instance().parseArgs(view.getActivityContext(),
    ...
    private void beginBeingsThreads(int beingCount) {
        ...
        mBeingsThreads = new ArrayList<>(beingCount);
        for (int index = 0; index < beingCount; index++) {
            mBeingsThreads.add(new edu.vandy.presenter.BeingThread(new edu.vandy.presenter.BeingRunnable(index, this), index, this));
        }

        for (edu.vandy.presenter.BeingThread beingThread : mBeingsThreads) beingThread.start();
    }
    ...
    private void waitForBeingsThreads() {
        ...
        new Thread(
                new Runnable() {
                    @Override
                    public void run() {
                        for (edu.vandy.presenter.BeingThread beingThread : mBeingsThreads) {
                            try {
                                beingThread.join();
    ...
    public void shutdown() {
        synchronized (this) {
            ...
            edu.vandy.presenter.BeingThread.shutdown();
